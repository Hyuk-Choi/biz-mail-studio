...
사용자가 메일 목적, 포함해야 할 내용, 기존 초안, 원하는 톤과 언어를 입력하면 전문적인 비즈니스 메일 형식으로 재작성합니다. 업무 요청, 일정 조율, 미팅 요청, 제안, 감사, 사과, 회신 독촉 등 실무에서 자주 쓰는 메일 케이스를 기준으로 본문 구조를 다르게 구성합니다.
...
- 톤 선택: 정중한, 간결한, 부드러운, 단호한, 설득력 있는, 친근하지만 전문적인, 격식 있는, 글로벌 비즈니스 스타일
- 추천 제목, 최종 메일 본문, 개선 포인트 생성
- 결과 재작성 옵션: 더 정중하게, 더 짧게, 더 설득력 있게, 영어로 변환, 한국어로 변환, 다시 생성
- 결과 복사 및 복사 완료 피드백
- OpenAI API 서버 연동 구조
